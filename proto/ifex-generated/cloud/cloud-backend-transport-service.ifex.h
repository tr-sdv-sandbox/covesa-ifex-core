...
major_version: 1
minor_version: 0
description: 'Cloud-side counterpart to backend_transport_service. Acts as a router
  - single instance handles multiple content_ids and routes messages to subscribers
  based on their content_id filter. Subscribers specify which content_id they want
  when subscribing; MQTT subscriptions are created on demand when first subscriber
  connects for a content_id. Partitioning enables horizontal scaling across vehicle
  fleet. Simple implementations use partition_id=0, total_partitions=1. Sequence numbers
  are per-vehicle monotonic; gaps indicate dropped messages.
...
- name: cloud_transport
  ...
  - name: persistence_t
    ...
    - name: VOLATILE
      value: 1
      description: 'Retry until delivered. Kept in memory, lost on shutdown.

        '
    - name: DURABLE
      value: 2
      description: 'Retry until delivered. Persisted on graceful shutdown.
  ...
  - name: transport_metadata_t
    description: 'Optional transport-provided metadata. All fields optional. New fields
  ...
  - name: send_request_t
    ...
    - name: content_id
      datatype: uint32
      description: Content ID for the message (e.g., 200=RPC, 201=discovery, 202=scheduler)
    - name: payload
      datatype: bytes
  ...
  - name: send_response_t
    ...
    - name: sequence
      datatype: uint64
      description: 'Per-vehicle sequence number. Monotonically increasing. Use to
        correlate with on_ack events; gaps indicate failures.
  ...
  - name: vehicle_info_t
    description: Information about a known vehicle
    members:
    - name: vehicle_id
      datatype: string
    - name: status
  ...
  - name: list_vehicles_response_t
    description: List of known vehicles
    members:
    - name: vehicles
      datatype: vehicle_info_t[]
      description: Array of vehicle info
  - name: message_subscription_filter_t
    description: 'Filter for on_vehicle_message subscription. Specifies which content_id
      to receive messages for. MQTT subscription to v2c/+/{content_id} is created
      on demand when first subscriber connects for that content_id.

      '
    members:
    - name: content_id
      datatype: uint32
      description: Content ID to subscribe to (e.g., 200=RPC, 201=discovery, 202=scheduler)
  methods:
  - name: send_to_vehicle
    description: 'Queue message for delivery to a specific vehicle. Non-blocking.
      Vehicle must belong to this partition (returns WRONG_PARTITION otherwise). Messages
      are queued if vehicle is offline (per persistence level). Use on_ack to track
  ...
  - name: get_channel_info
    description: 'Get channel binding info for this instance. Includes content_id
  ...
  - name: list_vehicles
    description: List all known vehicles with their status
    output:
    - name: result
  ...
  - name: on_vehicle_message
    description: 'Message received from a vehicle. Subscriber specifies content_id
      when subscribing; all messages on this stream are for that content_id (implicit).
      MQTT subscription to v2c/+/{content_id} is created on demand. Only delivers
      messages for vehicles assigned to this partition. Sequence numbers are per-vehicle;
  ...
  - name: on_ack
    description: 'Delivery confirmation for message sent to vehicle. Only successful
      deliveries generate acks. Gaps in sequence numbers indicate failed deliveries.
  ...
  - name: on_vehicle_status
    description: 'Vehicle online/offline status change. Triggered by MQTT LWT or heartbeat
      timeout.
