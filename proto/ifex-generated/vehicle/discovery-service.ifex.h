...
major_version: 1
minor_version: 0
description: Service discovery and registry for dynamic service binding and capability
  detection
namespaces:
- name: registry
  ...
  - name: schema_entry_t
    description: Schema hash to full schema mapping
    members:
    - name: schema_hash
      datatype: string
      description: SHA-256 hash of IFEX schema
  ...
  - name: heartbeat
    ...
    - name: status
      datatype: service_status_t
      description: Current service status
  - name: get_service_hashes
    description: Get SHA-256 hashes of all registered service schemas (for sync protocol)
    output:
    - name: hashes
      datatype: service_hash_t[]
      description: List of service name to schema hash mappings
  - name: get_schemas_by_hash
    description: Get full IFEX schemas for the requested hashes
    input:
    - name: hashes
      datatype: string[]
      description: SHA-256 hashes to retrieve
    output:
    - name: schemas
