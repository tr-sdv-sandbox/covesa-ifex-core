...
private:
    static std::string get_build_dir() {
        ...
        if (fs::exists(build_dir / "reference-services" / "discovery" / "vehicle" / "service" / "ifex-discovery-service")) {
        ...
        if (fs::exists(cwd_build / "reference-services" / "discovery" / "vehicle" / "service" / "ifex-discovery-service")) {
        ...
        while (!current.empty() && current != current.root_path()) {
            ...
            if (fs::exists(try_build / "reference-services" / "discovery" / "vehicle" / "service" / "ifex-discovery-service")) {
    ...
    static pid_t start_discovery_service(int port) {
        ...
        std::string executable = build_dir + "/reference-services/discovery/vehicle/service/ifex-discovery-service";
    ...
    static pid_t start_dispatcher_service(int port, const char* discovery_addr) {
        std::string build_dir = get_build_dir();
        std::string executable = build_dir + "/reference-services/dispatcher/vehicle/service/ifex-dispatcher-service";
        ...
        LOG(INFO) << "Started dispatcher service with PID " << pid;
        return pid;
    }

    static pid_t start_scheduler_service(int port, const char* discovery_addr) {
        std::string build_dir = get_build_dir();
        std::string executable = build_dir + "/reference-services/scheduler/vehicle/service/ifex-scheduler-service";
