...
The RPC Protocol enables synchronous method invocation from cloud to vehicle. Unlike Discovery (static schemas) and Scheduler (async jobs), RPC provides request-response semantics with timeout guarantees.
...
All RPC messages use **content_id=200**.

| Direction | Topic Pattern | Payload |
|-----------|---------------|---------|
| c2v | `c2v/{vehicle_id}/200` | `rpc_request_t` |
| v2c | `v2c/{vehicle_id}/200` | `rpc_response_t` |
...
## Comparison: RPC vs Scheduler

| Aspect | RPC (200) | Scheduler (202) |
|--------|-----------|-----------------|
