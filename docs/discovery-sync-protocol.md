...
All messages use `content_id=201` with a discriminated union:

```
discovery_envelope {
    oneof payload {
        HashList    service_hashes   = 1;  // List of schema hashes
        SchemaMap   schemas          = 2;  // Hash → YAML mapping
    }
}

HashList {
    repeated string hashes = 1;  // SHA-256 hex strings (64 chars each)
}
