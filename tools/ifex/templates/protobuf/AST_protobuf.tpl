...
{# Convert reference-specs/service/common/foo.ifex.yml → common/foo.proto #}
{# Handle both old (reference-specs/common/) and new (reference-specs/*/common/) paths #}
{% for inc in item.includes|default([]) %}
{%- set inc_path = inc.file -%}
{%- set stripped = inc_path|replace('reference-specs/', '')|replace('specs/', '') -%}
{%- set parts = stripped.split('/') -%}
{%- if parts|length > 2 -%}
  {#- New path format: service/category/file.ifex.yml → category/file.proto -#}
  {%- set proto_path = parts[-2] + '/' + parts[-1]|replace('.ifex.yml', '.proto')|replace('.yml', '.proto') -%}
{%- else -%}
  {#- Old path format: category/file.ifex.yml → category/file.proto -#}
  {%- set proto_path = stripped|replace('.ifex.yml', '.proto')|replace('.yml', '.proto') -%}
{%- endif -%}
import "{{ proto_path }}";
{% endfor %}
